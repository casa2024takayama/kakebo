...
## 13. 実装履歴・主要変更点（v0.2〜v0.4.22）

本書執筆後の段階的実装で確定・補正された仕様を時系列に記録する。新しい仕様は本セクションが本文を上書きする。

### v0.2 系：Phase 1 ピボット
- 4請求グループ（PayPay/セゾン/イオン/JCB）と Card 型導入
- `billingCycle.ts` 締め日・引落日計算
- `forecast.ts` 引落予定・赤字判定
- Dashboard 全面書換、Cards 画面新規

### v0.3 系：Sprint 1
- `payCycle.ts` 給料日サイクル計算
- 新画面「タイムライン」
- 取引一覧の3軸表示（利用日／請求期間／引落日）
- `Transaction.date` 利用日固定、`actualWithdrawalDate` 派生扱い
- `billingPeriod` 導入（`billingMonth` は deprecated 互換維持）

### v0.4 系：機能拡張・バグ修正
- v0.4.0: タイムラインリッチ化（2層構造、シェブロン継続性表現、カード10色）
- v0.4.2: CSVインポート履歴
- v0.4.3: 全データバックアップ/復元、`actualWithdrawalDate` 自動付与
- v0.4.4: テストモード
- v0.4.7: イオンCSVプリセット、日付パーサ強化
- v0.4.8: bulkマイグレーション、ダッシュボード「今月残り」+「翌月」2カード化
- v0.4.10: bulk 上書きインポート（再取込で重複防止）
- v0.4.12: 「お支払い日」ラベル変種対応
- v0.4.13-16: ダッシュボード Stage 1-4（給料日カード・引落リスト・進行中サイクル・統合）
- v0.4.17: フィルタ統一（引落日基準固定）
- v0.4.18: カード以外データは「利用日=引落日」、computeDerivedDates 統一
- v0.4.19: 進行中サイクルに非カード反映、カード/非カード分離表示
- v0.4.20: forecast.ts も `getCycleByWithdrawalDate` 統一
- v0.4.21: 引落月オフセット（当月/翌月/翌々月）対応、ニコス（旧シェル）補正
- v0.4.22: pre-pushフック、タイムラインキーボードナビ、フェードアニメ

### 実装で確定した社長指示（D5以降）
- D5: テストモードはページロード毎に取引・固定費をクリアし、カード/グループ/設定は保持
- D6: 「今日の残高」=月収（手取り）を流用（新フィールド追加せず）
- D7: 取引一覧の「現/次サイクル」フィルタは引落日基準で固定（viewModeはソートのみ影響）
- D8: 非カード取引は利用日=引落日として扱う（住宅ローン・サブスク等）
- D9: 楽天Master の「26日以降利用→翌々月扱い」は2カード方式で回避（特殊ロジック未実装）
- D10: bulkレコードは再取込時に同cardId+billingMonth で上書き（履歴重複防止）
- D11: 手動bulk入力時はカテゴリ選択不要（categoryId='' で保存）

### 既知の積み残し（v0.5以降検討）
- **Cashflow画面**（design-reference の3ペイン版）：Q1=B（新タブ）で承認済み、未着手
- **テスト導入**（Vitest+Unit）：QA仕様書あり、未着手
- **マルチデバイス同期**（Supabase）：Phase 2 中核
- **銀行CSV→固定費自動検出**：Phase 3
- **その他カードCSVプリセット**（楽天/JCB/JAL）：セゾン・イオンのみ実装済み
- **タイムライン明細⇔集約モード切替**：UI仕様書記載、未実装
